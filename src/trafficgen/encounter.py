...
def generate_encounter(desired_encounter_type, own_ship, target_ships,
                       target_ship_id, beta_default, relative_speed_default,
                       vector_time_default, settings):
    """
    ...
    while encounter_found != 1 and outer_counter < 5:
        ...
        own_ship_position_future = \
            calculate_position_at_certain_time(own_ship['start_pose']['position'],
                                               own_ship['start_pose']['speed'],
                                               own_ship['start_pose']['course'], vector_time)
        ...
        target_ship_position_future = \
            assign_future_position_to_target_ship(own_ship_position_future,
        ...
        while encounter_found != 1 and inner_counter < 5:
            ...
            if relative_speed is None:
                min_target_ship_speed = m_pr_min_2_knot(calculate_min_vector_length_target_ship(
                    own_ship['start_pose']['position'], own_ship['start_pose']['course'],
                    target_ship_position_future, beta)/vector_time)

                target_ship['start_pose']['speed'] = \
                    assign_speed_to_target_ship(desired_encounter_type, own_ship['start_pose']['speed'],
                                                min_target_ship_speed,
                                                settings['relative_speed'])
            else:
                target_ship['start_pose']['speed'] = relative_speed * own_ship['start_pose']['speed']
            target_ship['start_pose']['speed'] = np.minimum(target_ship['start_pose']['speed'],
                                                            target_ship['static']['speed_max'])

            target_ship_vector_length = knot_2_m_pr_min(target_ship['start_pose']['speed'])*vector_time
            start_position_target_ship, position_found = \
                find_start_position_target_ship(own_ship['start_pose']['position'], own_ship['start_pose']['course'],
                                                target_ship_position_future, target_ship_vector_length, beta,
                                                desired_encounter_type, settings)

            if position_found == 1:
                target_ship['start_pose']['position'] = start_position_target_ship
                target_ship['start_pose']['course'] = calculate_ship_course(target_ship['start_pose']['position'],
                                                                            target_ship_position_future)
                encounter_ok = check_encounter_evolvement(own_ship, own_ship_position_future,
                                                          target_ship, target_ship_position_future,
                                                          desired_encounter_type, settings)

                # Check if trajectory passes land
                trajectory_on_land = path_crosses_land(target_ship['start_pose']['position'],
                                                       target_ship['start_pose']['speed'],
...
def check_encounter_evolvement(own_ship, own_ship_position_future, 
                               target_ship, target_ship_position_future, 
                               desired_encounter_type, settings):
    """
    ...
    encounter_preposition_target_ship = \
        calculate_position_at_certain_time(target_ship_position_future, target_ship_speed,
                                           target_ship_course, -evolve_time)
    encounter_preposition_own_ship = \
        calculate_position_at_certain_time(own_ship_position_future, own_ship_speed,
                                           own_ship_course, -evolve_time)
    pre_beta, pre_alpha = \
        calculate_relative_bearing(encounter_preposition_own_ship, own_ship_course,
                                   encounter_preposition_target_ship, target_ship_course)
    pre_colreg_state = determine_colreg(pre_alpha, pre_beta,
                                        theta13_criteria, theta14_criteria,
                                        theta15_criteria, theta15)

    return 1 if pre_colreg_state == desired_encounter_type else 0


def calculate_min_vector_length_target_ship(own_ship_position, own_ship_course,
                                            target_ship_position_future, desired_beta):
    """
    ...
    p_2 = np.array([own_ship_position['north']+np.cos(psi),
                    own_ship_position['east'] + np.sin(psi)])
...
def find_start_position_target_ship(own_ship_position, own_ship_course,
                                    target_ship_position_future, target_ship_vector_length,
                                    desired_beta, desired_encounter_type, settings):
    """
    ...
    b = -2*e_2*e_4 - 2*n_2*n_4 + 2*e_1*e_2 + 2*n_1*n_2 + 2*e_1*(e_4-e_1) + 2*n_1*(n_4-n_1)
    ...
    if b**2-4*a*c > 0:
        ...
        target_ship_course_1 = calculate_ship_course({'north': n_31, 'east': e_31},
                                                     target_ship_position_future)
        beta1, alpha1 = calculate_relative_bearing(own_ship_position, own_ship_course,
                                                   {'north': n_31, 'east': e_31},
                                                   target_ship_course_1)
        colreg_state1 = determine_colreg(alpha1, beta1, theta13_criteria, theta14_criteria,
                                         theta15_criteria, theta15)
        target_ship_course_2 = calculate_ship_course({'north': n_32, 'east': e_32},
                                                     target_ship_position_future)
        beta2, alpha2 = calculate_relative_bearing(own_ship_position, own_ship_course,
                                                   {'north': n_32, 'east': e_32},
                                                   target_ship_course_2)
        colreg_state2 = determine_colreg(alpha2, beta2, theta13_criteria, theta14_criteria,
                                         theta15_criteria, theta15)
        if desired_encounter_type.lower() == colreg_state1 and\
           np.abs(beta1 - desired_beta % 360) < deg_2_rad(0.1):
            start_position_target_ship = {'north': n_31, 'east': e_31}
            start_position_found = 1
        elif desired_encounter_type.lower() == colreg_state2 and\
             np.abs(beta1 - desired_beta % 360) < deg_2_rad(0.1):  # noqa: E127
...
def determine_colreg(alpha, beta, theta13_criteria, theta14_criteria, theta15_criteria, theta15):
    ...
    if (alpha0360 > theta15[0]) & (alpha0360 < theta15[1]) & \
       (abs(beta0180) - theta13_criteria <= 0.001):
        return 'overtaking-give-way'
    if (abs(beta0180)-theta14_criteria <= 0.001) & (abs(alpha)-theta14_criteria <= 0.001):
        return 'head-on'
    if (beta > 0) & (beta < theta15[0]) & (alpha > -theta15[0]) & \
       (alpha-theta15_criteria <= 0.001):
        return 'crossing-give-way'
    if (alpha0360 > 0) & (alpha0360 < theta15[0]) & (beta0180 > -theta15[0]) & \
       (beta0180-theta15_criteria <= 0.001):
        return 'crossing-stand-on'
    return 'noRiskCollision'


def calculate_relative_bearing(position_own_ship, heading_own_ship,
                               position_target_ship, heading_target_ship):
    """
    ...
    if e_own_ship == e_target_ship:
        ...
        if e_own_ship < e_target_ship:
            if n_own_ship <= n_target_ship:
                bng_own_ship_target_ship = 1/2*np.pi - np.arctan(abs(n_target_ship-n_own_ship) /
                                                                 abs(e_target_ship-e_own_ship))
            else:
                bng_own_ship_target_ship = 1/2*np.pi + np.arctan(abs(n_target_ship-n_own_ship) /
                                                                 abs(e_target_ship-e_own_ship))
        else:
            if n_own_ship <= n_target_ship:
                bng_own_ship_target_ship = 3/2*np.pi + np.arctan(abs(n_target_ship-n_own_ship) /
                                                                 abs(e_target_ship-e_own_ship))
            else:
                bng_own_ship_target_ship = 3/2*np.pi - np.arctan(abs(n_target_ship-n_own_ship) /
                                                                 abs(e_target_ship-e_own_ship))
...
def calculate_ship_course(waypoint_0, waypoint_1):
    ...
    """
    course = np.arctan2(waypoint_1['east'] - waypoint_0['east'],
                        waypoint_1['north'] - waypoint_0['north'])
    if course < 0:
...
def assign_speed_to_target_ship(encounter_type, own_ship_speed, min_target_ship_speed,
                                relative_speed_setting):
    """
    ...
    if min_target_ship_speed/own_ship_speed > relative_speed[0] and \
       min_target_ship_speed/own_ship_speed < relative_speed[1]:
        relative_speed[0] = min_target_ship_speed/own_ship_speed

    return (relative_speed[0] + random.uniform(0, 1) *
            (relative_speed[1] - relative_speed[0])) * own_ship_speed
...
def update_position_data_target_ship(ship, lat_lon_0):
    ...
    lat, lon, _ = flat2llh(ship['start_pose']['position']['north'], 
                           ship['start_pose']['position']['east'],
                           deg_2_rad(lat_0), deg_2_rad(lon_0))
    ship['start_pose']['position'] = {'north': ship['start_pose']['position']['north'],
...
def update_position_data_own_ship(ship, lat_lon_0, delta_time):
    ...
    ship_position_future = calculate_position_at_certain_time(ship['start_pose']['position'], 
                                                              ship['start_pose']['speed'],
                                                              ship['start_pose']['course'], 
                                                              delta_time)
    lat, lon, _ = flat2llh(ship['start_pose']['position']['north'], ship['start_pose']['position']['east'],
                           deg_2_rad(lat_0), deg_2_rad(lon_0))
    lat_future, lon_future, _ = flat2llh(ship_position_future['north'],
    ...
    ship['start_pose']['position'] = {'north': ship['start_pose']['position']['north'],
                                      ...
                                      'latitude': round(rad_2_deg(lat), 6),
                                      'longitude': round(rad_2_deg(lon), 6)}
    ship['waypoints'] = [{'latitude': round(rad_2_deg(lat), 6),
                          'longitude': round(rad_2_deg(lon), 6)},
                         {'latitude': round(rad_2_deg(lat_future), 6),
                          'longitude': round(rad_2_deg(lon_future), 6)}]
