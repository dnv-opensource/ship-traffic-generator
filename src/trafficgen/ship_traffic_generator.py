...
def generate_traffic_situations(situation_folder, own_ship_file, target_ship_folder, settings_file):
    ...
    for _, desired_traffic_situation in enumerate(desired_traffic_situations):
        ...
        for _ in range(num_situations):
            ...
            own_ship = update_position_data_own_ship(own_ship, encounter_setting['lat_lon_0'],
            ...
            for k in range(len(desired_traffic_situation['encounter'])):
                desired_encounter_type = \
                    desired_traffic_situation['encounter'][k]['desired_encounter_type']
                settings = encounter_setting
                beta = find_value(desired_traffic_situation['encounter'][k], 'beta')
                relative_speed = find_value(desired_traffic_situation['encounter'][k],
                                            'relative_speed')
                vector_time = find_value(desired_traffic_situation['encounter'][k],
                                         'vector_time')
                target_ship_id = k+1
                target_ship, encounter_found = \
                    generate_encounter(desired_encounter_type, copy.deepcopy(own_ship),
                                       target_ships, target_ship_id, beta, relative_speed,
                                       vector_time, settings)
