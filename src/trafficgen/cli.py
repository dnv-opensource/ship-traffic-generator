...
from . import generate_traffic_situations
from . import plot_traffic_situations
from . import plot_specific_traffic_situation
from . import write_traffic_situations_to_json_file
...
@click.command()
@click_log.simple_verbosity_option(logger)
@click.option('-s', '--situations',
              help='Folders with situations (default=./baseline_situations_input/)',
              type=click.Path(exists=True),
              default=sit_folder, show_default=True)
@click.option('-os', '--own_ship',
              help='Path to own ship file',
              type=click.Path(exists=True),
              default=own_ship_file, show_default=True)
@click.option('-t', '--targets',
              help='Folder with target configurations',
              type=click.Path(exists=True),
              default=target_ship_folder, show_default=True)
@click.option('-o', '--output',
              help='Output folder',
              type=click.Path(exists=False),
              default=None, show_default=True)
@click.option('-c', '--settings',
              help='Path to settings file)',
              type=click.Path(exists=True),
              default=settings_file, show_default=True)
...
@click.option('--visualize-situation', type=int,
              help='Plot individual traffic situation, specify an INTEGER value larger than 0.')

def gen_situation(situations, own_ship, targets, settings, visualize, col, row,
                  visualize_situation, output):
    ...
    try:
        if visualize_situation > 0:
            ...
            plot_specific_traffic_situation(generated_traffic_situations,
                                            visualize_situation)
        else:
            click.echo("Invalid traffic situation number specified, not creating map plot. See --help for more info.")  # noqa: E501
    except TypeError:
        pass  # do nothing, value not defined, so we safely ignore this parameter :)

    if output is not None:
        click.echo("Writing traffic situations to files")
        write_traffic_situations_to_json_file(generated_traffic_situations,
                                              write_folder=output)
